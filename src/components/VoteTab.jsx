...
export default function VoteTab({ match, onXPEarned }) {
  ...
  useEffect(() => {
    async function load() {
      const [v, c] = await Promise.all([getUserVote(user.id, match.id), getVoteCounts(match.id)])
      setVoted(v); setCounts(c); setLoading(false)
    }
    load()
    const t = setInterval(() => getVoteCounts(match.id).then(setCounts), 10_000)
