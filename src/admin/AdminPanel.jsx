...
function MatchesSection({ matches, teams, reload, flash }) {
  ...
  async function runPayout(match) {
    if (!window.confirm(`Run payout for ${match.team_a?.short_name} vs ${match.team_b?.short_name}? This cannot be undone.`)) return
    try {
      const result = await adminRunPayout(match.id)
      flash(`✓ Paid: ${result.votes_paid} votes, ${result.predictions_paid} predictions`)
...
function QuestionsManager({ match, onUpdated, flash }) {
  ...
  async function load() {
    const qs = await adminGetQuestions(matchId)
    setQuestions(qs)
  }
  ...
  async function addQuestion() {
    ...
    try {
      ...
      await adminCreateQuestion({ match_id: matchId, question_key: qform.question_key.trim(), question_text: qform.question_text.trim(), options: opts })
      ...
      {questions.map(q => {
        ...
        return (
          <div key={q.id} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, marginBottom: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#fff', marginBottom: 8 }}>{q.question_text}</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {opts.map((opt, i) => (
                <button key={`${q.id}-${i}`} onClick={() => setCorrect(q.id, opt)} style={{
                  ...
                  cursor: 'pointer', fontFamily: 'inherit', fontWeight: q.correct_answer === opt ? 700 : 400,
                }}>
